...
| 屏 → 外壳 | `{type:'settheme', theme}` | 设置里改主题色 → 外壳 applyTheme 并广播（对应 Flutter 改 ThemeData） |
| 屏 → 外壳 | `{type:'setmode', appearance}` | 外观模式 light/dark/system（system 用 `matchMedia(prefers-color-scheme)` 解析） |
...
| 底部弹层 `.sheet`（`DZ.sheet`：动作菜单/选择器/轻表单） | `showModalBottomSheet`（圆角顶 + 拖拽柄 + `SafeArea` 底部留白）；单选用 `ListTile`+`trailing: check`，表单用内嵌 `Column`+`FilledButton` |
| 设置选择器 → `settheme/setmode` | 设置页改 `ThemeData`，全树 rebuild；「跟随系统」= `ThemeMode.system` + `MediaQuery.platformBrightness` |
| 回忆卡片 `memory.html`（预览 + 画幅/风格切） | 卡片即 widget，`RepaintBoundary.toImage()` → PNG → `share_plus` / 存相册；画幅/风格 = widget 参数 |
| 回收站 `trash.html`（软删 / 恢复 / 彻底删） | 一条按 `deleted_at` 筛选的查询；恢复 = 清 `deleted_at`，彻底删 = 真 DELETE；30 天后台阶清理 |
| 日历 `calendar.html`（月视图 + 当日条目） | `table_calendar` 或自绘 `GridView`；有条目日点查询同「月份计数」，选中日加载当日条目列表 |
