/**
 * check-changelog.js · CHANGELOG 卫生防漂移扫描（DayZ）
 *
 ...
 *                 docs/{DESIGN-REF,PROTOTYPE-ARCH,BACKLOG}.md（只点名，不 fail）
 *
 * ─────────────────────────────────────────────────────────────
...
const MAX_SUB    = 3;     // 单条目允许的子 bullet 上限（CLAUDE.md 深度上限：1 行标题 + 最多 3 子 bullet）
...
if (fatEntries.length > 0) {
  ...
  log(`    深内容指向 DESIGN-REF.md / PROTOTYPE-ARCH.md / BACKLOG.md，条目里只留一句话 + 指路。`);
} else {
