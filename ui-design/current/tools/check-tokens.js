/**
 * check-tokens.js · token 纪律防漂移扫描（DayZ）
 *
 * 用途：扫描原型/规范里的 .css/.js/.jsx/.html，找违反 token 纪律的代码：
 *   ❌ 裸 hex          `#abc` / `#abcdef` / `#abcdef88`
 *   ❌ 裸 rgba         `rgba(0,0,0,.5)`
 *   ❌ 假 fallback     `var(--x, #fff)` / `var(--x, rgba(...))`
 *                     （允许 `var(--x, var(--y))` token→token fallback）
 *
 * CLAUDE.md 约定：颜色一律 `var(--*)`；中性色随 data-mode、强调色随
 * data-theme+data-mode。本脚本把这条「自觉纪律」变成可机检的 DoD 守卫。
 *
 * ─────────────────────────────────────────────────────────────
 *  扫哪些 / 跳哪些
 * ─────────────────────────────────────────────────────────────
 *  自动遍历 SCAN_ROOTS（不硬编码文件名 → 新增屏不漏扫、不需维护清单）。
 *  跳过：
 *    · 任何 `tokens.css`           —— token 唯一真源，hex/rgba 合法定义于此
 *    · app-icons/                  —— 品牌图标 artwork，固定调色板、非主题 token
 *    · img / 资产目录里的非代码文件 —— 按扩展名只收 .css/.js/.jsx/.html
 *
 * ─────────────────────────────────────────────────────────────
 *  baseline 机制
 * ─────────────────────────────────────────────────────────────
 ...
 *  要把当前所有违规重新固化为 baseline，把 args 设成 ['--write-baseline']。
 *
 * ─────────────────────────────────────────────────────────────
 *  怎么跑：read_file 本文件 → 整个粘到 run_script。
 *  只用沙箱 helper：readFile / saveFile / ls / log。
 * ═════════════════════════════════════════════════════════════*/
...
const SCAN_ROOTS = ['design-system', 'pages', 'prototype-kit'];
const ROOT_FILES = ['index.html'];          // 项目根的散件
const CODE_EXT   = /\.(css|js|jsx|html)$/i;  // 只收这些扩展名

// 整目录级 skip（品牌 artwork / 图片资产 / 归档）
const SKIP_DIRS = new Set(['img', '_archive']);
...
const strip  = (s, ext) => ext === '.css' ? stripCss(s) : ext === '.html' ? stripHtml(s) : stripJs(s);
...
if (writeBaseline) {
  ...
  if (!baseline) {
    ...
    if (news.length > 0) {
      ...
      log(`\n修法：`);
      log(`  1. 优先把 hex / rgba 收编进 design-system/assets/tokens.css（推荐）`);
