...
```
日期：2026-05-29
@Ray 直接拍板选 A 替代正式三件事实测；实测数据待补录。
A·iOS：①待补录 ②待补录 ③待补录
A·Android：①待补录 ②待补录 ③待补录
是否触发一票否决：否（@Ray 拍板选 A，未触发）
核查人：@Ray
```
...
实测说明：T4/T5 正式三件事实测数据待补录，@Ray 拍板替代实测以解时间盒。
核查人：@Ray
```
...
### 关键证据
- **@Ray 直接拍板**：到期以拍板替代正式三件事实测（T4/T5），按 R4 时间盒规则解盘，缺失的实测数据标注「待补录」。
...
### 实测待补声明
T4/T5 的正式三件事真机实测（iOS/Android × 拖图/缩放混排/中文 IME）数据缺失，由 @Ray 拍板替代实测以解时间盒；如后续集成期发现 A 图片交互或中文 IME 体验不达标，按归档返工规则另立 spec 处理（归档目录只读、不复活）。
