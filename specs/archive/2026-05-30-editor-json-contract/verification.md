...
- [x] 仅光标/选区/占位等编辑专属装饰存在差异，无内容差异 — 已由 `render_consistency_test.dart` 自动化测试与代码审计确认（@Ray）
...
## 回归检查
- [x] `content_plain` 第一行作为标题与 data-layer/搜索消费方约定一致（取值方式未漂移） — 经代码与 spec 联检确认一致（@Ray）
- [x] 契约层不引入对编辑页 UI / PDF 导出全流程的反向依赖（保持「被依赖」单向） — 经 import 依赖图扫描审计确认（@Ray）
