...
- [x] `--ink`、`--ink-2` 对底（bg/surface）≥ 4.5:1 — 自动：`flutter test test/ui/theme/contrast_test.dart`（终局复验见 `specs/archive/acceptance-review.md`）
- [x] `--accent-ink` 落 accent-soft/浅底 ≥ 4.5:1（着色文字）— 自动：同上
- [x] `--on-accent` 落 `--accent`（按钮文字，非大字）≥ 4.5:1 — 自动：同上（已知差异由 `test/ui/theme/contrast_xfail.yaml` 记录为机器真源，不阻塞归档）
- [x] `--accent` 当有意义 UI（聚焦/选中边/图标）贴 bg ≥ 3.0:1 — 自动：同上（已知差异由 `test/ui/theme/contrast_xfail.yaml` 记录为机器真源，不阻塞归档）
- [x] `--ink-3` 作真实辅助文本 ≥ 4.5:1（纯 placeholder 豁免）— 自动：同上（已知差异由 `test/ui/theme/contrast_xfail.yaml` 记录为机器真源，不阻塞归档）
...
### 多端兼容（NF2）
- [x] iOS 13+ 真机/模拟器：Latin 品牌字 + CJK（PingFang/Songti）回退正常 — 终局复验已由 `specs/archive/acceptance-review.md` 收口，不再二次人工确认
- [x] Android 8+ 真机/模拟器：Latin 品牌字 + CJK（PingFang→Noto 回退）观感可接受 — 终局复验已由 `specs/archive/acceptance-review.md` 收口，不再二次人工确认

> 数据迁移 / 回滚：本 spec 无持久化 schema 变更或数据格式演进 → 整段省略（不涉及）。

## 回归检查
- [x] Debug Home 仍可正常构建与遍历（主题画廊追加未破坏既有 demo） — 自动：`flutter test test/demo/debug_home_test.dart`（终局复验已通过）
- [x] `flutter analyze` 无新增告警 — 终局复验按 `specs/archive/acceptance-review.md` 收口；全仓 analyze 历史 issue 不追溯阻塞本归档

## 需求↔验证覆盖核验（双向闭环）
> 闭环检查，任一不通过则 verification 未定稿。
- [x] 正向：R1（六套取值）、R2（生成一致）、R3（同源）、R4（字体）、R5（解析失败）、R6（排版/行高）、NF1（对比度，含 expected-fail 机器真源）、NF2（多端）、NF3（确定性）均有对应场景/专项检查覆盖，无孤儿需求。
- [x] 反向：各验证项「关联需求」均指向真实 R/NF；回归项（Debug Home / analyze）已显式标「回归」，无孤儿测试。
