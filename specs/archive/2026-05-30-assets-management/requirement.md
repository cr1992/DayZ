...
> 选档依据：5 个专项维度逐维过一遍，结论全「否」→ 取精简档（单模块、纯本地构建工具，跨多模块维由 design `## 文件变更` 客观判定，本 spec 文件变更均落在根 `pubspec.yaml` / `flutter_gen.yaml` / `assets/` / `lib/gen/`，属单模块）。

| 专项维度 | 命中？ | 依据（一句话） |
|---|---|---|
| 安全 | 否 | 仅本地构建期生成类型安全资源引用类，不涉及密钥/加密/网络/敏感数据。 |
| 权限 | 否 | 不读写需授权资源（相册/相机/文件系统外部），仅打包内置静态资产。 |
| 无障碍 | 否 | 不产出任何 UI/可交互界面，无对比度/语义标签等无障碍约束。 |
| 性能 | 否 | 生成在构建期一次性完成，运行期引用为常量字段，无运行时性能阈值。 |
| 多端兼容 | 否 | `flutter_gen` 产物为纯 Dart 常量类，各端等价编译，无端差异需专项验证。 |
