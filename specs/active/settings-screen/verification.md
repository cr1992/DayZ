...
| Debug Home 入口 | 进 Debug Home → 设置屏 demo | 入口可达、假数据渲染、可切换换肤、红线文案可读 | R7 | 自动 + 人工 |
...
- [ ] `lib/ui/settings/*.dart` 与 `lib/demo/settings_screen_demo.dart` 不 import `package:.../data/`/`lib/data` 内部 DAO、不 import Drift、不持 Drift 句柄、不出现 SQL/`PRAGMA rekey`/直接密钥操作 — 自动：`flutter test test/ui/settings/settings_repo_boundary_test.dart`（断言本屏依赖图不含 data/Drift；用 Dart 分析/导入图断言，**非** grep 被改文件自身文本——断言来源于编译/导入关系这一独立可观测事实）

> 数据迁移 / 回滚：本 spec 无持久化 schema 变更或数据格式演进（本屏不碰 DB，偏好/统计经外壳与底层 spec）→ 整段省略（不涉及）。

## 回归检查
- [ ] Debug Home 仍可正常构建与遍历（设置屏 demo 追加未破坏既有 demo） — 自动：`flutter test test/demo/debug_home_test.dart`（回归）
- [ ] `flutter analyze` 无新增告警 — 自动：`flutter analyze`（回归）

## 需求↔验证覆盖核验（双向闭环）
> 闭环检查，任一不通过则 verification 未定稿。
- [ ] 正向：R1（结构）、R2（主题换肤）、R3（外观换肤）、R4（加密只读）、R5（媒体红线）、R6（回调上抬）、R7（Debug Home）、NF1（样式参数）、NF2（44px/对比度/Semantics/reduce-motion）、NF3（多端）、NF4（Repository 边界）均有对应场景/专项检查覆盖，无孤儿需求。
