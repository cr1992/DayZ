...
- 结果：该日格变选中实底；下方区显示选中日日期头（「M 月 D 日」+「周X · N 篇」，日期/计数走 `intl`/`AppLocalizations`）+ 该日条目列表（每项标题 + 摘要 + 收藏星状态）；点某条目项 → 经 `Routes.reader` 携该 entry id 导航进阅读屏。
...
- 结果：下方显示日期头 +「这一天没有记录」类空态文案（`AppLocalizations`），无条目卡片、无异常（对应原型 `.cm-none`）。
...
### NF4 · Semantics 标签
月份导航钮、回到今天钮、返回钮、日格、条目项 MUST 提供无障碍语义标签（`AppLocalizations` 集中），屏幕阅读器可识别用途；日格语义须含「日期 + 有/无条目 + 是否今日/选中」信息。
