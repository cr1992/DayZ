...
- **UI 文案国际化默认约束**：未来新增或修改 UI / 屏幕 spec 时，用户可见文案与 Semantics 标签的唯一来源是 `lib/l10n/arb/app_zh.arb` + `lib/l10n/arb/app_en.arb`，运行期经 `AppLocalizations.of(context)` / `l10n.xxx` 取用，并按 `gen-l10n` 更新生成产物。`AppStrings` 已废弃，MUST NOT 新增、追加或在新任务白名单中继续作为文案落点；历史引用统一由 `ui-i18n-migration` 迁移。
