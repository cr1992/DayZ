...
- **状态**：功能生命周期，本表为唯一来源。
- **进度**：派生自 tasks.md 的快照，**可选**。若保留，**用命令现算、不手动维护**（如 `grep -c '^- \[x\]' tasks.md` 与任务总数之比），避免又一个手动同步的易漂移副本。README 真正需要准的是「状态」列，精确进度想知道时去 tasks.md 数即可。
...
7. **同步 README 状态**：功能生命周期发生**跨阶段变化**时，立即更新 README 的「状态」列——开始第一个任务时「草稿 → 进行中」；满足完成判定时「进行中 → 已完成」并按归档流程移行；废弃时「→ 已废弃」。这是「何时该碰 README」的明确触发点，不依赖 AI 自行想起。（逐任务的进度数字不在此列：进度若保留，按 README 列说明用命令现算，不手动更新。）
