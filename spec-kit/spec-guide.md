...
> **结构合规由 `spec-kit/scripts/lint_spec_structure.sh` 强制**（pre-commit 硬闸）：task 须用 `- [ ]` 复选框、**禁 `## T{n}` 标题反模式**；inline 字段（关联需求 / 依据设计 / 可改文件）与 `### 验收方式` 子段须在；标准档 requirement 须有 `## 非功能需求`、design 须有 `## 文件变更` / `## 已知风险`。注意：lint 是**高信号粗闸**，绿仅证「该段/字段存在 ≥1」，**不证完整也不证非空**——结构是否真合规仍须对照本模板逐节自检（lint 绿 ≠ 结构合规）。
...
- **原生跨界流程别一律推给人工**：权限 / 相册 / 文件 IO / Keychain / 前后台等过去只能人工走查的，现可用 Patrol E2E 自动化（用例放 `patrol_test/`、`dependsOn e2e-harness`，见其 spec）。但**安全·不可逆链路（加密 / 备份 / 还原）即便 E2E 绿仍保留人工终验**——patrol 有静默假阳性先例，验收时校验 `Total:` 非零。
...
两种状态各有唯一来源，不互相复制：
- **功能生命周期**（草稿/待实现/进行中/已完成/已废弃）：唯一来源是 `README.md`，由 spec 定稿与 tasks 完成情况驱动。
...
| 状态 | 含义 | 是否参与执行选取 |
|---|---|---|
| 草稿 | requirement/design/tasks/verification 仍在收敛，允许大改 | 否 |
| 待实现 | 四件套已定稿、依赖登记完整、lint/自查通过，尚未开始执行任务 | 是 |
| 进行中 | 已开始至少一个 task，或已有实现/验收改动在推进 | 是 |
| 已完成 | tasks 全 `[x]`，verification 全部完成并满足归档/维护态条件 | 否 |
| 已废弃 | 取消、被取代或明确不再做 | 否 |

`文档状态` 只表达单个文档成熟度，取值仅 `草稿 / 定稿`；不要把 `待实现`、`进行中`、`已完成` 等生命周期状态写进文件头。
...
| [feature-b](active/feature-b/) | P1 | feature-a | 待实现 | @yourname | YYYY-MM-DD |
| [feature-c](active/feature-c/) | P2 | feature-a | 草稿 | @yourname | YYYY-MM-DD |
...
- **状态**：功能生命周期，本表为唯一来源；取值为 `草稿 / 待实现 / 进行中 / 已完成 / 已废弃`。active 表不应保留 `已完成` / `已废弃` 这类终态行，终态 spec 应移入对应归档/废弃位置。

依赖就绪/被阻塞（派生，不另设列）：一个 spec **依赖就绪** = 其「依赖」列所有前置 spec 状态均为「已完成」；否则**被阻塞**。该状态由「依赖」列与各前置「状态」派生，无需手填。

不设「进度」列：精确进度派生自 tasks.md，静态表格无法动态计算，手动维护必漂移。欲知进度直接看对应 `tasks.md` 的勾选情况。README 只对「状态」负责。

执行选取规则：在「待实现 / 进行中」**且依赖就绪**（依赖列前置全部已完成）的 spec 中，挑**优先级最高**的执行；同级则按创建顺序。`草稿` 与被阻塞的 spec 不参与选取，直到其文档定稿且前置全部完成。
...
7. **同步 README 状态**：功能生命周期发生**跨阶段变化**时，立即更新 README 的「状态」列——四件套定稿且自查/lint 通过时「草稿 → 待实现」；开始第一个任务时「待实现 → 进行中」；满足完成判定时「进行中 → 已完成」并按归档 checklist 移行（优先用 `archive_spec.sh`）；废弃时「→ 已废弃」。这是「何时该碰 README」的明确触发点。README 不含进度数字，无逐任务同步负担。
